    ...
    methods
        ...
        function o = GeneralFitFunctions(type,xdata,ydata)
            ...
            if strcmp(o.type, 'Sinus')
                ...
                startfreq = 1./(abs(o.xdata(xmax)-o.xdata(xmin))*2);
                o.fitFunction = @(p,x) p(1)+p(2).*exp(-p(3).*x).*sin(p(4)+2*pi*p(5)*x);
                
                o.paramNames =         {    'offset',   'amp1',       'damp1',     'pha1',   'fre1'};
                
                o.startParams = double([    mean(o.ydata);     max(o.ydata)./2-min(o.ydata)./2;   0; 0;  startfreq]);
                o.lowerBound  = double([0; 0.0;      0;   -(2*pi+0.05);   startfreq./5]);
                o.upperBound  = double([max(o.ydata); 2*max(o.ydata);   4E-2;    (2*pi+0.05);   5*startfreq]);
