...
filterOptions = [];
fftCutoff = 3;
fitOptions = optimset('MaxFunEvals',1000,'TolFun',1e-8,'MaxIter',1000,'TolX',1e-20,'display','none');
fitname = 'Band pass 8th order 10kHz to 14kHz';

foldername = sprintf('%s%.0e_%.0e_%u_fft%u/', fitname, fitOptions.TolFun, fitOptions.TolX,  smoothing, fftCutoff);
...
%%
files = {...
    '2016_02_04_Double_quench_11kHz9_2kHz_complete.mat'...
...
for iFile=numel(files):-1:22
    clearvars -except iFile files M STD fitOptions filterOptions foldername smoothing P dataPath fftCutoff;
    [~, name, ~] = fileparts(files{iFile});
    currentFile = ['fitresults/' foldername name '.mat'];
    currentFileBare = [dataPath 'bare/' name '.mat'];
    %% load files
    ...
    if exist(currentFileBare, 'file') == 2
    ...
    %% saving
    save(currentFile, 'dataStructure', 'P' );
    
    %% save image
    
end
clearvars -except iFile files M STD fitOptions filterOptions foldername smoothing P dataPath;
save(['fitresults/' foldername 'b']);